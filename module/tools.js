...
const fs = require("fs");
//英数字
const eisuji = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
//ひらがな
...
//ロシア文字
const rosia = "АБВГДЕЁабвгдеёЖЗИЙКЛМжзийклмНОПРСТУнопрстуФХЦЧШЩЪфхцчшщъЫЬЭЮЯыьэюя";
//罫線
const keisen = "─│┌┐┘└├┬┤┴┼━┃┏┓┛┗┣┳┫┻╋┠┯┨┷┿┝┰┥┸╂";
//入力可能文字
const inputPossibleChar = eisuji + hiragana + katakana + kanjiStandard1 + kanjiStandard2 + girisya + rosia + keisen;
...
let tools = {
    ...
    checkDate(strData) {
        try {
            if (strData == null || strData.length == "") {
                return true;
            }
            var y = strData.substr(0, 4);
            var mark1 = strData.substr(4, 1);
            var m = strData.substr(5, 2);
            var mark2 = strData.substr(7, 1);
            var d = strData.substr(8);
            var date = new Date(y, m, d);
            if (date.getFullYear() != y || date.getMonth() != m || date.getDate() != d || mark1 != "/" || mark2 != "/") {
                return false;
            }
            return true;
        } catch (error) {
    ...
    // 英数字チェック 英数字じゃない場合、false
    checkApNo(strData) {
        if (strData == null || strData == "") {
            return true;
        }
        if (strData.match(/^[A-Za-z0-9]*$/)) {
            return true;
        } else {
            return false;
        }
    ...
    // 数字チェック 数字じゃない場合、false
    checkNo(strData) {
        if (strData == null || strData == "") {
            return true;
        }
        if (strData.match(/^[0-9]*$/)) {
            return true;
        } else {
            return false;
        }
    ...
    // 桁数チェック 桁数超える場合、false
    checkSize(strData, size) {
        if (strData == null || strData == "") {
            return true;
        }
        if (strData.length > size) {
            return false;
        } else {
            return true;
        }
    ...
    // 入力可能文字チェック 禁則文字がある場合、false
    checkInputChar(strData) {
        for (var i = 0, len = strData.length; i < len; i++) {
            // 入力可能文字
            if (inputPossibleChar.indexOf(strData.substr(i, 1)) < 0) {
                // 禁則文字がある場合
                return false;
            }
        }
